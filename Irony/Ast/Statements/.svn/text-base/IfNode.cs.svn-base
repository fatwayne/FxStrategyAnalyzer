...
namespace Irony.Ast {
  public class IfNode : AstNode {
    ...
    public override void EvaluateNode(EvaluationContext context, AstMode mode, DateTime time) {
      Test.Evaluate(context, AstMode.Write, time);
      var result = context.Data.Pop();
      if (context.Runtime.IsTrue(result)) {
        if (IfTrue != null)    IfTrue.Evaluate(context, AstMode.Read, time);
      } else {
        if (IfFalse != null)   IfFalse.Evaluate(context, AstMode.Read, time);
      }
    }
  }//class
