...
namespace Irony.Ast {
  public class StringTemplateNode : AstNode {
      ...
      for(int i = 0; i < _segments.Count; i++) {
        ...
        switch(segment.Type) {
          ...
          case SegmentType.Expression: 
            values[i] = EvaluateExpression(context, segment, new DateTime());
    ...
    private string EvaluateExpression(EvaluationContext context, TemplateSegment segment, DateTime time) {
      try {
          segment.ExpressionNode.Evaluate(context, AstMode.Read, time);
