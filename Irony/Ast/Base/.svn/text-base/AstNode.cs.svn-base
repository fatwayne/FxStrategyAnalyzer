...
namespace Irony.Ast {
  ...
  public delegate void NodeEvaluate(EvaluationContext context, AstMode mode, DateTime time); 

  //a base class for NodeConfig classes - container for custom settings for a node; you attach an instance
  // to NonTerminal together with AstNodeType, and it will be available in AstNode.Init method.
  ...
  //Base AST node class
  public partial class AstNode : IAstNodeInit, IBrowsableAstNode, IInterpretedDateTimeAstNode
  {
    ...
    public virtual void Evaluate(EvaluationContext context, AstMode mode, DateTime time) {
      try {
        EvaluateRef(context, mode, time); 
    ...
    public virtual void EvaluateNode(EvaluationContext context, AstMode mode, DateTime time)
    {
    }
